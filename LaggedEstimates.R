...
get_delayed_estimates = function(data_in, delay_day){
    ...
    model5 = tidy(data_in %>% felm(open ~  Feb_Avg + prop_home_device_zip |  newfactor + postal_code + date | (proption_BigBrands_naics_postal_open  ~ BrandPostalProp) | countyName,.)) %>% mutate(delay = delay_day, Obs = totalObs)
...
out2$term[out2$term == "`proption_BigBrands_naics_postal_open(fit)`"] = "Prop. Brand Est. Open"
...
ggsave("plots/delay/delay_FE.jpg",p1)
ggsave("plots/delay/delay2_FFE.jpg",p2)
