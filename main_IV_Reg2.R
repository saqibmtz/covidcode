...
### Controls; With FE ####
fs3 <- data_nb %>% felm(proption_BigBrands_naics_postal_open  ~ BrandPostalProp + Feb_Avg + prop_home_device_zip| newfactor + postal_code + date| 0 | countyName,.)
iv3 <- data_nb %>% felm(open ~  Feb_Avg + prop_home_device_zip |  newfactor + postal_code + date | (proption_BigBrands_naics_postal_open  ~ BrandPostalProp) | countyName,.)
...
screenreg(list(fs2,iv2,fs3,iv3),digits=3,caption = "Regression",caption.above = T,custom.header = list("Model 6" = 1:2, "Model 7" = 3:4),custom.model.names = c("First Stage","IV","First Stage","IV"),reorder.coef=c(4,1,2,3),custom.gof.rows=list("Fixed Effect NAICS"=c("Yes","Yes","No","No"),"Fixed Effect PostalCode" = c("Yes","Yes","Yes","Yes"), "Fixed Effect Date"=c("Yes","Yes","Yes","Yes"),"Fixed Effect NAICS XX Date"=c("No","No","Yes","Yes"),"Fixed Effect County XX Date"=c("No","No","No","No")),table=F)

texreg(list(fs2,iv2,fs3,iv3),digits=3,file = "tables/table2b.tex",caption = "Regression",caption.above = T,custom.header = list("Model 6" = 1:2, "Model 7" = 3:4),custom.model.names = c("First Stage","IV","First Stage","IV"),reorder.coef=c(4,1,2,3),custom.gof.rows=list("Fixed Effect NAICS"=c("Yes","Yes","No","No"),"Fixed Effect PostalCode" = c("Yes","Yes","Yes","Yes"), "Fixed Effect Date"=c("Yes","Yes","Yes","Yes"),"Fixed Effect NAICS XX Date"=c("No","No","Yes","Yes"),"Fixed Effect County XX Date"=c("No","No","No","No")),table=F)
